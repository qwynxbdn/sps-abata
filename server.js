...
app.post("/api/login", async (req, res) => {
  ...
  try {
    ...
    if (userRes.rows.length === 0)
      return res
        .status(401)
        .json({ ok: false, error: "Username tidak ditemukan atau non-aktif" });
    ...
    const roleRes = await pool.query(
      "SELECT PermissionsJson FROM Roles WHERE Role = $1",
      [user.role],
    );
...
app.get("/api/me", authenticateToken, async (req, res) => {
  try {
    const userRes = await pool.query(
      "SELECT UserId, Name, Username, Role FROM Users WHERE UserId = $1 AND IsActive = TRUE",
      [req.user.userId],
    );
  ...
  async (req, res) => {
    ...
    try {
      const cpRes = await pool.query(
        "SELECT * FROM Checkpoints WHERE BarcodeValue = $1 AND Active = TRUE",
        [barcode],
      );
      if (cpRes.rows.length === 0)
        return res
          ...
          .json({
            ok: false,
            error: "QR Code tidak dikenali atau Checkpoint sedang non-aktif",
          });
      ...
      const p1 = (lat * Math.PI) / 180,
        p2 = (cp.latitude * Math.PI) / 180;
      const dp = ((cp.latitude - lat) * Math.PI) / 180,
        dl = ((cp.longitude - lng) * Math.PI) / 180;
      const a =
      ...
      const notes = isWithinRadius
        ? ""
        : `Diluar radius (${Math.round(distance)}m > ${cp.radiusmeters}m)`;
      ...
      if (resultStatus === "SUCCESS") {
        res.json({
          ok: true,
          data: { status: resultStatus, distance: Math.round(distance) },
        });
      } else {
  ...
  async (req, res) => {
    try {
      const result = await pool.query(
        "SELECT UserId, Name, Username, Role, IsActive, CreatedAt, LastLoginAt FROM Users ORDER BY CreatedAt DESC",
      );
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (err.code === "23505")
        return res
          .status(400)
          .json({ ok: false, error: "Username sudah digunakan" });
      res.status(500).json({ ok: false, error: err.message });
    }
  },
);
...
app.get(
  ...
  async (req, res) => {
    try {
      const result = await pool.query(
        "SELECT * FROM Checkpoints ORDER BY CreatedAt DESC",
      );
      res.json({ ok: true, data: result.rows });
    } catch (err) {
      res.status(500).json({ ok: false, error: err.message });
...
app.post(
  ...
  async (req, res) => {
    const { Name, BarcodeValue, Latitude, Longitude, RadiusMeters, Active } =
      req.body;
    try {
      await pool.query(
      ...
      if (err.code === "23505")
        return res
          ...
          .json({
            ok: false,
            error: "Barcode/QR Value sudah digunakan titik lain",
          });
      res.status(500).json({ ok: false, error: err.message });
    }
  },
...
app.get(
  ...
  async (req, res) => {
    try {
      ...
      const result = await pool.query(
        "SELECT LogId, Timestamp, Username, BarcodeValue, DistanceMeters, Result, Notes FROM PatrolLogs ORDER BY Timestamp DESC LIMIT 200",
      );
      res.json({ ok: true, data: result.rows });
    } catch (err) {
      res.status(500).json({ ok: false, error: err.message });
...
app.get("/api/reports/pdf", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=Laporan-Patroli-${month}-${year}.pdf`,
    );
    pdfDoc.pipe(res);
    pdfDoc.end();
  } catch (err) {
